...
def main() -> int:
    ...
    try:
        ...
        env2["ECOAIMS_FRONTEND_PORT"] = os.getenv("ECOAIMS_FRONTEND_PORT", "8071")
        env2["ECOAIMS_FRONTEND_HOST"] = env["HOST"]
        env2["ECOAIMS_REQUIRE_CANONICAL_POLICY"] = "true"
        ...
        try:
            ...
            p = subprocess.run([sys.executable, "scripts/smoke_browser_tabs_playwright.py"], env={"ECOAIMS_FRONTEND_URL": fe_url})
            _assert(p.returncode == 0, "browser smoke gagal")
            print("PASS canonical_crossrepo: browser smoke")
            return 0
        finally:
            proc_fe.terminate()
            try:

...
def main() -> int:
    ...
    dispatch_proof = str(os.getenv("ECOAIMS_SMOKE_DISPATCH_PROOF", "false")).strip().lower() in {"1", "true", "yes", "y", "on"}
    dispatch_screenshot_path = str(os.getenv("ECOAIMS_SMOKE_DISPATCH_SCREENSHOT", "output/dispatch_proof.png")).strip() or "output/dispatch_proof.png"
        ...
        for profile in run_profiles:
            ...
            for vp in viewports:
                for _ in range(max(1, iterations)):
                    ...
                    if dispatch_proof and (not expect_neg_error):
                        page.get_by_text("BMS", exact=True).click()
                        page.locator("#bms-rl-run-btn").wait_for(timeout=p_timeout_ms)
                        page.locator("#bms-rl-run-btn").click()
                        page.locator("#bms-rl-table").wait_for(timeout=p_timeout_ms, state="attached")
                        page.locator("#bms-rl-table").scroll_into_view_if_needed()
                        page.locator("#bms-rl-table .column-header-name").filter(has_text="grid_import_kwh").wait_for(timeout=p_timeout_ms, state="attached")
                        try:
                            os.makedirs(os.path.dirname(dispatch_screenshot_path) or ".", exist_ok=True)
                        except Exception:
                            pass
                        page.screenshot(path=dispatch_screenshot_path, full_page=True)
                    ...
                    page.locator("#forecast-period-dropdown").wait_for(timeout=p_timeout_ms)
                    page.locator("#forecast-consumption-graph").wait_for(timeout=p_timeout_ms)
                    ...
                    page.locator("#reports-period-dropdown").wait_for(timeout=p_timeout_ms)

                    page.get_by_text("About", exact=True).click()
                    page.locator("#about-runtime-info").wait_for(timeout=p_timeout_ms)
